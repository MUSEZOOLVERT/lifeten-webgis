...
Install:
    Add `SERIALIZATION_MODULES = { 'geojson' : 'path.to.geojson_serializer' }`
    to your project ``settings.py`` file.
...
class Serializer(OverloadedSerializer):
    ...
    def end_serialization(self):
        simplejson.dump(self.objects, self.stream, cls=DjangoGEOJSONEncoder,
                        **self.options)
...
def Deserializer(stream_or_string, **options):
    ...
    def GEOJsonToEWKT(dict):
        ...
        The default decoder would pass our geo dict object to the constructor
        which would result in a TypeError; using the below hook we are forcing
        it into a ewkt format. This is accomplished with a class hint as per
        JSON-RPC
        """
        # using class hint catch a GEOSGeometry definition
        if '__GEOSGeometry__' in dict:
